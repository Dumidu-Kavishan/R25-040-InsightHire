...
const ScreenRecorder = forwardRef(({ 
  ...
  const currentClipIdRef = useRef(null); // Store the clip_id for the current 30-second recording
  ...
  const startAudioClipRecording = (stream) => {
    try {
      ...
      // Generate a unique clip_id for this 30-second recording
      const clipId = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      currentClipIdRef.current = clipId;
      console.log('🆔 Generated clip_id for this recording:', clipId);
    ...
    try {
      ...
      // Get the clip_id generated at the start of recording
      const clipId = currentClipIdRef.current;
      console.log('🆔 Using clip_id from recording start:', clipId);
      
      const audioClipData = {
        clip_id: clipId,  // Send the clip_id generated by frontend
