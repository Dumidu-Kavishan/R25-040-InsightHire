...
# ==================== TRIAL ACCOUNT API ENDPOINTS ====================

@app.route('/api/trial/create', methods=['POST'])
def create_trial_account():
    """Create a new trial account"""
    try:
        from services.trialAccountService import TrialAccountService
        
        trial_service = TrialAccountService()
        result = trial_service.create_trial_account()
        
        if result['success']:
            return jsonify({
                'status': 'success',
                'message': 'Trial account created successfully',
                'trial_account': result['trial_account']
            })
        else:
            return jsonify({
                'status': 'error',
...
def get_trial_status():
    """Get trial account status for user"""
    try:
        user_id = get_user_id_from_request()
...
@app.route('/api/trial/check-interview-limit', methods=['GET'])
def check_interview_limit():
    ...
    try:
        ...
        trial_service = TrialAccountService()
        result = trial_service.check_interview_limit(user_id)
...
@app.route('/api/trial/check-job-role-limit', methods=['GET'])
def check_job_role_limit():
    ...
    try:
        ...
        trial_service = TrialAccountService()
        result = trial_service.check_job_role_limit(user_id)
