...
class PretrainedVoiceDetector:
    ...
    def _calculate_emotion_scores(self, features, audio_data, sample_rate):
        ...
        happy_score = 0
        if energy > 0.02: happy_score += 3
        if spectral_centroid > 1000: happy_score += 2
        if pitch > 150: happy_score += 2
        if hnr > 0.3: happy_score += 2
        if rhythm_regularity > 0.6: happy_score += 1
        scores['happy'] = happy_score
        
        # SAD: Low energy, low pitch, irregular rhythm
        sad_score = 0
        if energy < 0.01: sad_score += 3
        if spectral_centroid < 800: sad_score += 2
        if pitch < 120: sad_score += 2
        if rhythm_regularity < 0.4: sad_score += 2
        if energy_variance < 0.005: sad_score += 1
        scores['sad'] = sad_score
        
        # ANGRY: High energy, high pitch variation, irregular patterns
        angry_score = 0
        if energy > 0.03: angry_score += 3
        if energy_variance > 0.02: angry_score += 2
        if zcr > 0.1: angry_score += 2
        if spectral_centroid > 1200: angry_score += 2
        if rhythm_regularity < 0.5: angry_score += 1
        scores['angry'] = angry_score
        
        # FEAR: High pitch, irregular energy, high variation
        fear_score = 0
        if pitch > 200: fear_score += 3
        if energy_variance > 0.015: fear_score += 2
        if zcr > 0.08: fear_score += 2
        if spectral_centroid > 1100: fear_score += 2
        if rhythm_regularity < 0.6: fear_score += 1
        scores['fear'] = fear_score
        
        # CALM: Moderate energy, low variation, regular rhythm
        calm_score = 0
        if 0.01 <= energy <= 0.025: calm_score += 3
        if energy_variance < 0.01: calm_score += 2
        if rhythm_regularity > 0.7: calm_score += 2
        if 0.05 <= zcr <= 0.08: calm_score += 2
        if hnr > 0.4: calm_score += 1
        scores['calm'] = calm_score
        
        # NEUTRAL: Balanced features
        neutral_score = 0
        if 0.008 <= energy <= 0.02: neutral_score += 2
        if 0.005 <= energy_variance <= 0.015: neutral_score += 2
        if 0.06 <= zcr <= 0.1: neutral_score += 2
        if 800 <= spectral_centroid <= 1200: neutral_score += 2
        if 0.5 <= rhythm_regularity <= 0.8: neutral_score += 2
